...
import os, math
try:
    from NSM.train.init import init_hybrid, init_nsm, init_parallel
    from NSM.train.evaluate_nsm import Evaluator_nsm
    from NSM.data.load_data_super import load_data
except:
    from UniModel.reader.NSM.train.init import init_hybrid, init_nsm, init_parallel
    from UniModel.reader.NSM.train.evaluate_nsm import Evaluator_nsm
...
class Trainer_KBQA(object):
    def __init__(self, args, logger=None):
        ...
        self.mode = "teacher"
        tp_model_name = self.args['model_name']
        # teacher_type = self.args['teacher_model']
        self.teacher_type = self.args['teacher_type']
        self.args['model_name'] = self.args['teacher_model']
        if self.teacher_type == "parallel":
            self.teacher = init_parallel(self.args, self.logger, len(self.entity2id), self.num_kb_relation,
                                         len(self.word2id))
        elif self.teacher_type == "hybrid":
            self.teacher = init_hybrid(self.args, self.logger, len(self.entity2id), self.num_kb_relation,
                                        len(self.word2id))
        elif self.teacher_type == "nsm":
            self.teacher = init_nsm(self.args, self.logger, len(self.entity2id), self.num_kb_relation,
                                    len(self.word2id))
        else:
            raise NotImplementedError
        self.args['model_name'] = tp_model_name
    ...
    def load_pretrain(self):
        ...
        if args['load_teacher'] is not None:
            ckpt_path = os.path.join(args['checkpoint_dir'], args['load_teacher'])
            print("Load ckpt from", ckpt_path)
            ...
            # model = self.teacher.model
            # model = self.student
            self.logger.info("Load param of {} from {}.".format(", ".join(list(model_state_dict.keys())), ckpt_path))
    ...
    def label_data_with_teacher(self):
        self.teacher.eval()
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            with torch.no_grad():
                middle_dist, label_valid = self.teacher.label_data(batch)
            tp_list = [dist.unsqueeze(1) for dist in middle_dist]
            # (B, 1, max_local_entity)
            cur_dist = torch.cat(tp_list, dim=1)
            # (B, num_step - 1, max_local_entity)
            if self.label_valid is None:
                self.label_valid = label_valid
                self.label_dist = cur_dist
            else:
                self.label_valid = torch.cat((self.label_valid, label_valid), dim=0)
                self.label_dist = torch.cat((self.label_dist, cur_dist), dim=0)
        self.logger.info("label training set done!")
    ...
    def train_epoch(self):
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            sample_ids = self.train_data.sample_ids
            # with torch.no_grad():
            #     middle_dist = self.teacher.label_data(batch)
            label_valid = self.label_valid[sample_ids]
            # (B, 1)
            label_dist = self.label_dist[sample_ids]
            # print(label_dist.size())
            # (B, num_step-1, max_local_entity)
            middle_dist = torch.chunk(label_dist, self.num_step - 1, dim=1)
            # print(middle_dist[0].size())
            self.optim_student.zero_grad()
            loss, extras, _, tp_list = self.student.train_batch(batch, middle_dist, label_valid)
