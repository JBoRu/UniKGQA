...
try:
    ...
    from UniModel.reader.NSM.Modules.Reasoning.base_reasoning import BaseReasoning
...
class GNNReasoning(BaseReasoning):
    ...
    def private_module_def(self):
        ...
        for i in range(self.num_step):
            if self.args["simplify_model"]:
                self.add_module('rel_linear' + str(i),
                                nn.Linear(in_features=self.plm_hidden_size, out_features=entity_dim))
            else:
    ...
    def reason_layer_wo_ent_emb(self, curr_dist, instruction, rel_linear):
        ...
        # print(fact_rel.shape, fact_query.shape)
        fact_val = torch.cosine_similarity(rel_linear(fact_rel), fact_query, dim=1).unsqueeze(dim=-1) # (num_fact, 1)
        # print(fact_val.shape)
    ...
    def forward(self, current_dist, relational_ins, step=0, return_score=False):
        ...
        if self.args["not_use_ent_emb"]:
            ...
            score_tp = score_tp + (1 - answer_mask) * VERY_NEG_NUMBER
            current_dist = self.softmax_d1(score_tp)
        # current_dist = self.sigmoid_d1(score_tp)
        if return_score:
