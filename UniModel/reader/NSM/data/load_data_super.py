...
try:
    ...
    from UniModel.reader.NSM.util.config import get_config
    from UniModel.reader.NSM.data.dataset_super import SingleDataLoader, SingleDataLoaderForPLM
...
def load_data(config):
    entity2id = load_dict(os.path.join(config['data_folder'], config['entity2id']))
    word2id = load_dict(os.path.join(config['data_folder'], config['word2id']))
    relation2id = load_dict(os.path.join(config['data_folder'], config['relation2id']))
