...
if check_gpus():
    ...
    class GPUManager():
        ...
        def __init__(self, priority_list, memory_free, qargs=[]):
            '''
            '''
            self.qargs = qargs
        ...
        def choose_no_task_gpu(self):
            ...
            for i in range(len(unspecified_gpus)):
                unsp_gpu = unspecified_gpus[i]
                if unsp_gpu['index'] in self.priority_list and unsp_gpu['memory.free'] > self.memory_free:
                    gpu_av.append(i)
